...
export function Notifications({ userId }: { userId: string }) {
  ...
  const { data, error, isPending, isError, fetchNextPage, isFetchingNextPage, hasNextPage } = useInfiniteQuery<
    GetActivity[],
    Error,
  ...
  // Polling replaced by SSE: useNotificationsSSE (in MenuBar) invalidates
  // the notifications query when the unread count increases, triggering
  // fetchPreviousPage via React Query's automatic refetch.
    ...
    <div>
      <div className="flex justify-between">
        ...
        <DropdownMenuButton
          ...
          label={t('notifications_option')}
          onAction={markAllAsRead}
          disabledKeys={disabledKeys}>
          <Section>
            <Item key="mark-all">{t('mark_all_as_read')}</Item>
          </Section>
        </DropdownMenuButton>
      </div>
      <div>
